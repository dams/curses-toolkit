...
#							  x2 => 50, y2 => 80,
	...
	$window->add_widget($label = Curses::Toolkit::Widget::Label->new()
						->set_text("Hello !                                                                           .\n                                                                           .\n                                                                           .\n                                                                           .\n                                                                           .\n\n\n")
					   );
	...
	$root->add_delay($t->(10), sub { $set->(" OK ! now back to business (Book, you can put your T-Shirt back on)\n") });
	...
	$move_audience_window_sub = sub { my $wc = $audience_window->get_coordinates(); 
									  $audience_window->set_coordinates(x1 => $wc->x1() + 1, x2 => $wc->x2(),
																		y1 => $wc->y1(), y2 => $wc->y2()
																	   );
									  $loop1-- and 	$root->add_delay(1/3, $move_audience_window_sub);
								  };
	$move_audience_window_sub2 = sub { my $wc = $audience_window->get_coordinates(); 
									  $audience_window->set_coordinates(x1 => $wc->x1() - 1, x2 => $wc->x2() + 1,
																		y1 => $wc->y1(), y2 => $wc->y2()
																	   );
	...
	$root->add_delay($t->(3), sub { $audience_window->set_theme_property(title_width => 50);
	...
	$root->add_delay($t->(4), sub { $audience_window->{theme} = Curses::Toolkit::Theme::Default::Color::Pink->new($audience_window);
	...
	my $loop4 = 20;
	my $sub4;
	$sub4 = sub { $audience_label->set_text("  resize me ! $loop4 secs");
	...
	$root->add_delay($t->(2), sub {
		...
		$vbox->pack_end( 
						$audience_entry = Curses::Toolkit::Widget::Entry
						->new_with_text('Enter some text')
						->set_name('entry1'),
						{ expand => 1 }
					   )
					 ->pack_end(my $hbox = Curses::Toolkit::Widget::HBox->new()
								->pack_end(
										   $audience_button = Curses::Toolkit::Widget::Button
										   ->new_with_label(' OK ')
										   ->set_name('button1'),
										   { expand => 0 }
										  ),
	...
	$root->add_delay($t->(1), sub { $set->("Please enter something in the entry and hit space on the button\n") });
...
$step3 = sub {
	...
	$root->add_delay($t->(2), sub {
						 ...
						 $hpane->add1(my $l1 = Curses::Toolkit::Widget::Label->new()
									  ->set_text('This is a naive label. Very naive')
									  ->set_name('label1'),
									 );
						 $hpane->add2(my $l2 = Curses::Toolkit::Widget::Label->new()
	...
	$root->add_delay($t->(2), sub { $audience_window->set_coordinates( x1 => 3, y1 => 3, x2 => 40, y2 => 10);
