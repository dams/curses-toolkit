...
sub main {
	...
	$vpaned->set_name('vpaned'),
	$vpaned->set_gutter_position(10);
	$hpaned->add1(Curses::Toolkit::Widget::Label->new()
				 ->set_text('This is a naive label. Very naive')
				 ->set_name('label1'),
				);
	$hpaned->add2($vpaned
				  ->add1(Curses::Toolkit::Widget::Label->new()
						 ->set_text('An other nonetheless naive label.Honest !')
						 ->set_name('label2'),
						)
				  ->add2(Curses::Toolkit::Widget::Label->new()
						 ->set_text('What ?! yet another naive Label ?!')
						 ->set_name('label3'))
				 );
